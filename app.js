...
var createTableIfNotExist = async function createTableIfNotExist(
	tableName,
	createFunction
) {
	var params = {
				...
				.then(() => {
					...
					if (tableName === "Tables_Data") {
						...
						dynamodb.putItem(init0, async (err, data) => {
							if (err) {
								console.error(
									`Unable to initialize Tables_Data len to 4 --> ${err}`
								);
							} else {
								console.log("Initialized Tables_Data len to 4.");
							}
						});
						...
						dynamodb.putItem(init1, async (err, data) => {
							if (err) {
								console.error(`Unable to initialize Users len to 0 --> ${err}`);
							} else {
								console.log("Initialized Users len to 0.");
							}
						});
						...
						dynamodb.putItem(init2, async (err, data) => {
							if (err) {
								console.error(
									`Unable to initialize Places len to 0 --> ${err}`
								);
							} else {
								console.log("Initialized Places len to 0.");
							}
						});
						...
						dynamodb.putItem(init3, async (err, data) => {
							if (err) {
								console.error(
									`Unable to initialize Reviews len to 0 --> ${err}`
								);
							} else {
								console.log("Initialized Reviews len to 0.");
							}
						});
					}
				})
...
var createTables = async function createTables() {
	...
	await createTableIfNotExist(
		"Tables_Data",
		Tables_DataCreateTable.createTables_DataTable
	);
};
...
//auth
const options = {
	// Used for certificate for HTTPS
	key: fs.readFileSync("host.key"),
	cert: fs.readFileSync("host.cert"),
};

https.createServer(options, app).listen(global.gConfig.port, function() {
	console.log(
		`${global.gConfig.app_name} listening on port ${global.gConfig.port}`
	);
});
/*
