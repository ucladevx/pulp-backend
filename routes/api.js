		...
		dynamodb.query(params, (err, friend) => {
			if (err) {
				...
				if (friend.Items.length != 0) {
					resolve(friend.Items[0].user_id.N);
				} else {
					resolve(null);
				}
			}
...
//find a user either using the user_id or the facebook_id
async function find_user_if_exists(params) {
	let user_id = params.user_id || "0";
	let facebook_id = params.facebook_id || "";
	...
	return new Promise((resolve, reject) => {
		...
		if (facebook_ids.length != 0) {
			get_pulp_id(facebook_ids[0])
				.then((pulp_id) => {
					if (pulp_id != null) {
						pulp_ids.push(pulp_id.toString());
					} else {
						console.log(
							`Could not find a pulp user with facebook id = ${facebook_ids[0]}`
						);
					}
	...
	return new Promise((resolve, reject) => {
		if (friends_pulp_ids.length == 0) {
			...
			if (friend_id == "0") {
				...
				// params to remove user_id from friends
				else {
					update_params = {
						TableName: "Users",
						Key: { user_id: { N: friend_id } },
...
router.post("/new_user", async (req, res) => {
	...
	//check if user exists with the facebook_id
	find_user_if_exists({ facebook_id: req.body.facebook_id })
		.then(async (data) => {
			//if user with facebook id not found, create new user
			if (!data) {
						...
						convert_to_pulp_ids(friends_facebook_ids)
											...
											.then(() => {
												...
												add_or_remove_from_friends(new_id, friends_pulp_ids, 1)
													...
													.catch((err) => {
														res
															.status(500)
															.send(`update_friends failed --> ${err}`);
													});
											})
											.catch((err) => {
												res
													.status(500)
													.send(
														`Increment table failed in new_user --> ${err}`
													);
											});
									}
								});
							...
							.catch((err) => {
								res
									.status(500)
									.send(
										`Unable to convert facebook IDs to pulp IDs --> ${err}`
									);
							});
					})
					.catch((err) => {
						res
							.status(500)
							.send(
								`Error getting Users table length from Tables_Data --> ${err}`
							);
					});
			}
			//user with facebook id already exists
				...
				res
					.status(500)
					.send(
						`User with facebook_id = ${req.body.facebook_id} already exists.`
					);
			}
		})
		.catch(async (err) => {
			...
			console.log(err);
			res.status(500).send(`Error while finding user in database`);
		});
});
...
router.get("/find_user", (req, res) => {
	if (req.query.user_id) {
		find_user_if_exists({ user_id: req.query.user_id })
			.then(async (user) => {
				if (!user) {
		...
		find_user_if_exists({ facebook_id: req.query.facebook_id })
			.then(async (user) => {
				if (!user) {
					res
						.status(500)
						.send(`No user with facebook_id = ${req.query.facebook_id}`);
				} else {
					res.json(user);
				}
			})
					...
					if (err2) {
						...
						if ("Attributes" in user) {
							...
							add_or_remove_from_friends(req.body.user_id, list_of_friends, 0)
								.then(() => {
									res.send(`User (${req.body.user_id}) has been destroyed.`);
								})
								.catch((err4) => {
									res
										.status(500)
										.send(`Failed to remove_from_friends --> ${err4}`);
								});
						} else {
							res
								.status(500)
		...
		if (err) {
			...
			if (user.Count == 0) {
				...
				let update_params = {
					...
					Key: { user_id: { N: req.body.user_id } },
					UpdateExpression:
						"set first_name = :first_name, last_name = :last_name, photo = :photo, places = :places, facebook_id = :facebook_id",
					ExpressionAttributeValues: {
						":first_name": { S: req.body.first_name },
						":last_name": { S: req.body.last_name },
						":photo": { S: req.body.photo },
				...
				dynamodb.updateItem(update_params, (err, user) => {
					if (err) {
						res
							.status(500)
							.send(`Could not update user (${req.body.user_id}) --> ${err}`);
					} else {
						res.send(
							`User (${req.body.user_id}) has been successfully edited.`
						);
					}
				});
			}
	...
	dynamodb.query(user_param, (err, user) => {
		if (err) {
			...
			if (user.Items.length != 0) {
				...
				// }
				let list = [];
				get_friend_places(friends, 0, place_ids).then((places) => {
					console.log(places);
					get_place_list(places, 0, friends, list).then((list) => {
						list.sort((a, b) =>
...
async function get_place_list(place, index, friends, list) {
	return new Promise((resolve, reject) => {
		if (index !== place.length) {
			get_place(place[index], friends).then((response) => {
				if (response) {
					list.push(response);
				}
				get_place_list(place, index + 1, friends, list).then((list) => {
					resolve(list);
				});
			});
		} else {
	...
	return new Promise((resolve, reject) => {
		if (index !== friends.length) {
			...
			dynamodb.query(friend_param, (err, friend) => {
				if (err) {
					console.log("Could not find friend in DB");
					get_friend_places(friends, index + 1, place_ids).then((place_ids) => {
						resolve(place_ids);
					});
				} else {
					if (friend.Items.length != 0) {
	...
	get_table_len(REVIEWS)
		.then((length) => {
				...
				if (err) {
					...
					increment_table_len(REVIEWS)
						...
						.catch((err) => {
							res
								.status(500)
								.send(`Increment table failed in add_review --> ${err}`);
						});
				}
			});
			...
			dynamodb.updateItem(params2, function(err, data) {
				if (err) {
					console.error(
						"Unable to add review id into reviews in Places; Error JSON: ",
					...
					res.send(
						`New review ${new_id} has been created and added into Places.`
					);
				}
			});
		})
		.catch((err) => {
			res
				.status(500)
				.send(`Error getting Reviews table length from Tables_Data --> ${err}`);
		});
});
...
router.post("/create_place", async (req, res) => {
	...
	get_table_len(PLACES)
				...
				if (err) {
					...
					increment_table_len(PLACES)
						...
						.catch((err) => {
							res
								.status(500)
								.send(`Increment table failed in create_place --> ${err}`);
						});
				}
			});
		})
		.catch((err) => {
			res
				.status(500)
				.send(`Error getting Places table length from Tables_Data --> ${err}`);
		});
});
...
router.post("/edit_place", async (req, res) => {
	...
	var params = {
		...
		Key: { place_id: { N: req.body.place_id } },
		UpdateExpression:
			"set p_name = :p_name, image = :image, city = :city, p_state = :p_state, address1 = :address1, address2 = :address2, zip_code = :zip_code, latitude = :latitude, longitude = :longitude, tags = :tags",
		ExpressionAttributeValues: {
			":p_name": { S: req.body.name },
			":image": { S: req.body.image },
			":city": { S: req.body.city },
	...
	dynamodb.updateItem(params, function(err, data) {
		if (err) {
			console.error(
				"Unable to edit place; Error JSON: ",
				JSON.stringify(err, null, 2)
			);
			res.status(500).send(`Error editing place`);
		} else {
	...
	dynamodb.query(user_params, async (err, user) => {
		if (err) {
			...
			if (user.Count == 0) {
				...
				await get_place(req.query.place_id, user.Items[0].friends.NS).then(
					(response) => {
		...
		if (index !== review_ids.length) {
			...
			dynamodb.query(review_params, async (err, review) => {
				if (err) {
					...
					//check if review found for that review_id
					// mainly to check that review_id = 0 does not yield any reviews
					if (review.Count !== 0) {
						reviews.push(review.Items[0]);
						if (fbfriends.includes(review.Items[0].postedBy.N.toString())) {
							// cast ID to string
							...
							var user_param = {
								...
								KeyConditionExpression: "user_id = :v1",
								ExpressionAttributeValues: {
									":v1": { N: review.Items[0].postedBy.N.toString() },
								},
							};
				...
				if (place.Items.length == 0) {
					...
					await get_reviews(
						...
						let update_params = {
							...
							ExpressionAttributeValues: {
								":val": { N: (arr[0] == 0 ? 0 : arr[0] / arr[1]).toString() }, // 0/0 should not take place
							},
						};
						dynamodb.updateItem(update_params, (err, data) => {
							if (err) {
								...
								let response = {
									place: place.Items[0],
									averageRating: arr[0] == 0 ? 0 : arr[0] / arr[1], // 0/0 should not take place
	...
	dynamodb.scan(params, function(place_err, place_data) {
		if (place_err) {
			console.error(
				...
				JSON.stringify(place_err, null, 2)
			);
			...
			dynamodb.getItem(user_params, function(user_err, user_data) {
				if (user_err) {
					...
					place_data.Items.forEach(async function(place) {
						...
						var customized_place = await get_place(
							place.place_id.N,
							user_data.Item.user_id.N,
							res
						);
